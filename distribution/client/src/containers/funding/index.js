...
class Funding extends PureComponent {
    ...
    getPlot = async () => {
        ...
        if (dataPin !== null) {
            ...
            asyncForEach(
                Array.from(Array(dataPin.length).keys()),
                async itemsG => {
                    let amountAtom = 0;
                    ...
                    asyncForEach(
                        ...
                        async item => {
                            ...
                            const address =
                                dataTxs[item].tx.value.msg[0].value
                                    .from_address;
                            const amou =
                                Number.parseInt(
                                    dataTxs[item].tx.value.msg[0].value
            ...
            await asyncForEach(
                ...
                async item => {
                    ...
                    table.push({
                        ...
                        amount:
                            Number.parseInt(
                                dataTxs[item].tx.value.msg[0].value.amount[0]
                                    .amount
                            ) *
                            10 ** -1,
                        estimation
        ...
        if (dataAllPin !== null) {
            await asyncForEach(
                ...
                async pin => {
                    for (let i = 0; i < groups.length; i++) {
                        if (
                            groups[i].group.indexOf(
                                `${dataAllPin[pin].group}`
                            ) !== -1
                        ) {
                            groups[i].pin = true;
                        }
    ...
    unPinItem = item => {
        ...
        if (allPin != null) {
            for (let i = 0; i < allPin.length; i++) {
                const tempindexItem =
                    allPin[i].group.indexOf(`${item.group}`) !== -1;
                if (tempindexItem) {
                    allPin.splice(i, 1);
                    localStorage.setItem("allpin", JSON.stringify(allPin));
                    this.updateList(allPin);
            ...
            <span>
                <main className="block-body">
                    ...
                    <Statistics
                        ...
                        won={formatNumber(
                            Math.floor(won * 10 ** -9 * 1000) / 1000
                        )}
                        price={formatNumber(
                            Math.floor(currentPrice * 10 ** -9 * 1000) / 1000
                        )}
                        discount={
                            Math.floor(currentDiscount * 100 * 1000) / 1000
                        }
                    />
