...
import { P2PChannelSender } from './P2PSyncChannel';
// import { IDBDatastore } from 'datastore-idb';
/* The line `import { MemoryDatastore } from 'datastore-core';` is importing the `MemoryDatastore`
class from the 'datastore-core' module. This class is likely used for creating an in-memory data
store, which can be used for storing data within the application's memory during runtime. This can
be useful for temporary storage or caching purposes where data needs to be accessed quickly without
...
export const createP2PApi = (broadcastApi: BroadcastChannelSender) => {
  ...
  const p2pChannleSender = P2PChannelSender();
  let node: Option<Libp2p>;
  let peerId: NeuronPeerId = '';
  ...
  const start = async (options: IpfsOptsType) => {
    try {
      ...
      node!.services!.pubsub!.addEventListener('message', (event) => {
        ...
        const json = JSON.parse(toString(data));
        if (json.type !== PUBSUB_CHAT_MESSAGE) {
          p2pChannleSender.post(json);
        } else {
          broadcastChannel.postP2PMessage(topic, json.message);
        }

        console.log(`[in] ${topic}`, json);
      });

      setTimeout(async () => {
        console.timeEnd('🔋 p2p initialized');
        // await connectPeer(
        //   '/dns4/ipfs-comet.cybernode.ai/tcp/443/wss/p2p/12D3KooWGAai4e9ZmeYFAuGyrmRWUTFyS1TSYoYu2ftFtFkwQsFs'
        // );
        connectPeer(CYBERNODE_PUBSUB_NODE)
          // .then(() => subscribeChannel(DEFAUL_P2P_TOPIC))
          .then(() => subscribeChannel(DEFAUL_P2P_SERVICE_TOPIC))
          .catch(console.error);
  ...
  const sendPubSubMessage = (
    topic: string,
    message: PubSubMessage & PubSubPeerId
  ) => {
    node!.services!.pubsub!.publish(topic, fromString(JSON.stringify(message)));
    console.log(`[out] [${topic}]`, message);
    broadcastApi.postP2PMessage(
      topic,
      `me: ${(message as P2PChatMessage).message}`
    );
  };
