...
const deleteStore = async (pathKey) => {
  try {
    const connection = indexedDB.open(pathKey, 2);
    connection.onsuccess = (e) => {
      const db = e.target.result;
      if (db.objectStoreNames.length > 0) {
        const transaction = db.transaction([pathKey], 'readwrite');
        const objectStore = transaction.objectStore(pathKey);
    ...
    console.log('error deleteStore', error);
  }
};
